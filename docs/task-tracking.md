...
the orchestrator (Basin) consumes it **under the hood** when running master/worker/judge.

Status: the `TaskStore`, `FilesTaskStore`, `VectorTaskStore`, verifier gate, external connector
seam, and `artesian task` CLI are implemented in `crates/headrace` and `artesian-cli`. The full
...
- Storage parity with memory: works with **markdown task files** *and* a **vector DB**, behind
  one trait — same dual-backend philosophy as Aquifer.
...
- **`VectorTaskStore`** — indexes each task's title+body into a Aquifer collection so the
...
- **`ExternalConnector`** — bridges to Jira/Linear through their MCP servers. Tasks created in
  Artesian are mirrored to the external tracker (and optionally pulled back), giving teams a visual
...
- Task tracking is a **separate component** (`crates/headrace`) so it can be used on its
...
- Storage parity with memory via a `TaskStore` trait; `FilesTaskStore` is the zero-infra default;
  `VectorTaskStore` reuses Aquifer; external trackers are connectors, never a hard dependency.
